...
import { sendDM, replyComment } from './instagram-api';
...
async function handleMessagingEvent(msg: any, account: InstagramAccount, event: WebhookEvent) {
    ...
    await evaluateWorkflows(account, channel, messageText, senderId, event, async (text) => {
        // Action: Send DM
        await sendDM(account.accessTokenEncrypted, senderId, text);
    });
}
...
async function handleChangeEvent(change: any, account: InstagramAccount, event: WebhookEvent) {
    if (change.field === 'comments') {
        ...
        if (fromId === account.igUserId) return; // Ignore self
        ...
        await evaluateWorkflows(account, 'FEED_COMMENT', text, fromId, event, async (replyText) => {
            await replyComment(account.accessTokenEncrypted, commentId, replyText);
        });
    }
}
...
async function evaluateWorkflows(
    ...
    senderId: string,
    event: WebhookEvent,
    executeAction: (text: string) => Promise<void>
) {
    // Find Workflows
    ...
    for (const wf of workflows) {
        // Check Channel
        const channels = wf.channels as any; // array
        if (!channels.includes(channel === 'STORY_REPLY' ? 'story' : (channel === 'DM_RECEIVED' ? 'dm' : 'feed'))) {
            ...
            if (!channels.includes(requiredChannel)) continue;
        }
        ...
        if (!trigger) continue; // Should exist if channel matches, but verify
        ...
        try {
            ...
            if (action) {
                const actionConfig = action.configJson as any;
